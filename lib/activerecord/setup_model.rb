module DraftPunk
  ...
  class SetupModel
    ...
    require_relative 'editable_diff_instance_methods'
    ...
    def make_approvable!
      ...
      @associations = @top_level_model.editable_target_associations if @associations.empty?
      ...
      @top_level_model.const_set :EDITABLE_NULLIFY_ATTRIBUTES, Array(@nullify).flatten.freeze
  ...
  private #####################################################################
    ...
    def setup_amoeba_for(target_model, options={})
      ...
      target_associations = target_model.editable_target_associations
      target_associations = set_valid_associations(target_model, target_associations)
      target_model.amoeba do
        enable
        include_associations target_model.const_get(:EDITABLE_VALID_ASSOCIATIONS) unless target_model.const_get(:EDITABLE_VALID_ASSOCIATIONS).empty?
      ...
      setup_editable_association_persistance_for_children_of target_model, target_associations
    end

    def setup_associations_and_scopes_for(target_model, set_default_scope: false, allow_previous_versions_to_be_changed: true)
      target_model.send       :include, Model::InstanceInterrogators unless target_model.method_defined?(:has_editable?)
      target_model.send       :attr_accessor, :temporary_approved_object
      target_model.send       :before_create, :before_create_editable if target_model.method_defined?(:before_create_editable)
      ...
      target_model.send       :include, Model::EditableDiffInstanceMethods
      ...
      target_model.has_one    :editable, -> { unscope(where: :approved) }, class_name: target_model.name, foreign_key: :approved_version_id
      target_model.scope      :editable, -> { unscoped.where("#{target_model.quoted_table_name}.approved_version_id IS NOT NULL") }
    end

    def setup_editable_association_persistance_for_children_of(target_model, target_associations=nil)
      target_associations = target_model.editable_target_associations unless target_associations
    ...
    def set_valid_associations(target_model, associations)
      return target_model.const_get(:EDITABLE_VALID_ASSOCIATIONS) if target_model.const_defined?(:EDITABLE_VALID_ASSOCIATIONS)
      ...
      target_model.const_set :EDITABLE_VALID_ASSOCIATIONS, valid_assocations.freeze
      valid_assocations
    end
  end
end
